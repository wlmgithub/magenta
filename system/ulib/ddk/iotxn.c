...
#define IOTXN_PFLAG_CLONE      (1 << 4)   // this txn is a clone
...
static void iotxn_release_free_list(iotxn_t* txn) {
    ...
    if (pflags & IOTXN_PFLAG_ALLOC) {
        ...
        if (!(pflags & IOTXN_PFLAG_CLONE) && virt) {
...
static mx_status_t iotxn_physmap_contiguous(iotxn_t* txn) {
    ...
    xprintf("physmap: txn %p vmo_offset 0x%" PRIx64 " page_offset 0x%" PRIx64 " paddr 0x%lx\n", txn, txn->vmo_offset, page_offset, sg->paddr);
...
mx_status_t iotxn_clone(iotxn_t* txn, iotxn_t** out) {
    ...
    clone->pflags |= IOTXN_PFLAG_CLONE;
