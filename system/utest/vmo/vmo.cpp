...
bool vmo_clone_test() {
    ...
    // test set 1: create a few clones, close them
    {
        ...
        EXPECT_EQ(NO_ERROR, mx_vmo_clone(vmo, MX_VMO_CLONE_PRIVATE, 0, size, &clone_vmo[0]), "vm_clone");
        EXPECT_NEQ(MX_HANDLE_INVALID, clone_vmo[0], "vm_clone_handle");

        // clone it a second time
        clone_vmo[1] = MX_HANDLE_INVALID;
        EXPECT_EQ(NO_ERROR, mx_vmo_clone(vmo, MX_VMO_CLONE_PRIVATE, 0, size, &clone_vmo[1]), "vm_clone");
        EXPECT_NEQ(MX_HANDLE_INVALID, clone_vmo[1], "vm_clone_handle");

        // clone the clone
        clone_vmo[2] = MX_HANDLE_INVALID;
        EXPECT_EQ(NO_ERROR, mx_vmo_clone(clone_vmo[1], MX_VMO_CLONE_PRIVATE, 0, size, &clone_vmo[2]), "vm_clone");
    ...
    // test set 2: create a clone, verify that it COWs via the read/write interface
    {
        ...
        for (size_t off = 0; off < size / 2; off += sizeof(off)) {
            size_t val = ~off;
            mx_vmo_write(clone_vmo[0], &val, off, sizeof(val), &bytes_handled);
        }
        ...
        for (size_t off = 0; off < size / 2; off += sizeof(off)) {
            ...
            if (val != ~off) {
                EXPECT_EQ(val, ~off, "vm_clone read back of clone");
                break;
            }
        }
    ...
    // test set 4: deal with clones with nonzero offsets and offsets that extend beyond the original
    {
        ...
        clone_vmo[0] = MX_HANDLE_INVALID;
        EXPECT_EQ(ERR_INVALID_ARGS, mx_vmo_clone(vmo, MX_VMO_CLONE_PRIVATE, 1, size, &clone_vmo[0]), "vm_clone");

        // create a clone that extends beyond the parent by one page
        clone_vmo[0] = MX_HANDLE_INVALID;
        EXPECT_EQ(NO_ERROR, mx_vmo_clone(vmo, MX_VMO_CLONE_PRIVATE, PAGE_SIZE, size, &clone_vmo[0]), "vm_clone");
