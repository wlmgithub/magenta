...
bool vmo_clone_test() {
    ...
    // test set 2: create a clone, verify that it COWs
    {
        mx_handle_t vmo;
        ...
        for (size_t off = 0; off < size; off += sizeof(off)) {
            mx_vmo_write(vmo, &off, off, sizeof(off), nullptr);
        }
        ...
        for (size_t off = 0; off < size; off += sizeof(off)) {
            ...
            mx_vmo_read(clone_vmo[0], &val, off, sizeof(val), nullptr);
        ...
        for (size_t off = 0; off < size; off += sizeof(off)) {
            size_t val = ~off;
            mx_vmo_write(clone_vmo[0], &val, off, sizeof(val), nullptr);
        }
        ...
        for (size_t off = 0; off < size; off += sizeof(off)) {
            ...
            mx_vmo_read(vmo, &val, off, sizeof(val), nullptr);
