...
static mx_status_t ums_inquiry(ums_t* msd, uint8_t* out_data) {
    ...
    if (status == NO_ERROR) {
        msd->data_iotxn->ops->copyfrom(msd->data_iotxn, out_data, UMS_INQUIRY_TRANSFER_LENGTH, 0);
    }
    return status;
}
...
static mx_status_t ums_request_sense(ums_t* msd, uint8_t* out_data) {
    ...
    mx_status_t status = ums_read_csw(msd, NULL);
    if (status == NO_ERROR) {
        msd->data_iotxn->ops->copyfrom(msd->data_iotxn, out_data, UMS_REQUEST_SENSE_TRANSFER_LENGTH, 0);
    }
    return status;
}
...
static mx_status_t ums_read_format_capacities(ums_t* msd, uint8_t* out_data) {
    ...
    mx_status_t status = ums_read_csw(msd, NULL);
    if (status == NO_ERROR) {
        msd->data_iotxn->ops->copyfrom(msd->data_iotxn, out_data, UMS_READ_FORMAT_CAPACITIES_TRANSFER_LENGTH, 0);
    }
    return status;
}
...
static mx_status_t ums_read_capacity10(ums_t* msd, scsi_read_capacity_10_t* out_data) {
    ...
    mx_status_t status = ums_read_csw(msd, NULL);
    if (status == NO_ERROR) {
        msd->data_iotxn->ops->copyfrom(msd->data_iotxn, out_data, sizeof(*out_data), 0);
    }
    return status;
}
...
static mx_status_t ums_release(mx_device_t* device) {
    ...
    if (msd->cbw_iotxn) {
        msd->cbw_iotxn->ops->release(msd->cbw_iotxn);
    }
    if (msd->data_iotxn) {
        msd->data_iotxn->ops->release(msd->data_iotxn);
    }
    if (msd->csw_iotxn) {
        msd->csw_iotxn->ops->release(msd->csw_iotxn);
    }
...
static int ums_worker_thread(void* arg) {
    ...
    while (1) {
        ...
        if (status >= 0) {
            txn->ops->complete(txn, NO_ERROR, status);
        } else {
            txn->ops->complete(txn, status, 0);
        }
    }
...
static mx_status_t ums_bind(mx_driver_t* driver, mx_device_t* device, void** cookie) {
   ...
   usb_endpoint_descriptor_t* endp = usb_desc_iter_next_endpoint(&iter);
    ...
    msd->data_iotxn = usb_alloc_iotxn(bulk_in_addr, PAGE_SIZE, 0);
    if (!msd->data_iotxn) {
        status = ERR_NO_MEMORY;
        goto fail;
    }
    msd->csw_iotxn = usb_alloc_iotxn(bulk_in_addr, sizeof(ums_csw_t), 0);
    if (!msd->csw_iotxn) {
        status = ERR_NO_MEMORY;
        goto fail;
    }
