...
typedef struct {
    ...
    int bulk_in_max_packet;
    int bulk_out_max_packet;
...
static void ums_queue_data_transfer(ums_t* msd, iotxn_t* txn, uint8_t ep_address, int max_packet) {
    iotxn_t* clone;
...
static void msd_async_read(mx_device_t* dev, mx_handle_t vmo, uint64_t length,
    ...
    ums_t* msd = get_ums(dev);
    iotxn_t* txn;
    mx_status_t status = iotxn_alloc(&txn, IOTXN_ALLOC_POOL, 0);
    if (status != NO_ERROR) {
    ...
    mx_vmo_get_size(vmo, &txn->vmo_length);
...
static mx_status_t ums_bind(mx_driver_t* driver, mx_device_t* device, void** cookie) {
    ...
    int bulk_in_max_packet = 0;
    int bulk_out_max_packet = 0;
