...
static mx_status_t xhci_start_transfer_locked(xhci_t* xhci, xhci_endpoint_t* ep, iotxn_t* txn) {
    ...
    if (txn->length > 0) {
        uint32_t sg_len;
        mx_status_t status = iotxn_physmap(txn, &state->sg, &sg_len);
        if (status != NO_ERROR) {
            return status;
        }
        state->sg_end = state->sg + sg_len;
    }
    ...
    usb_setup_t* setup = (state->ep_index == 0 ? &proto_data->setup : NULL);
    if (setup) {
...
static void xhci_get_phys(xhci_transfer_state_t* state, mx_paddr_t* out_phys, size_t* out_phys_length) {
    MX_DEBUG_ASSERT(state->sg != state->sg_end);
    iotxn_sg_t* sg = state->sg++;

    *out_phys = sg->paddr;
    *out_phys_length = sg->length;
}
...
static mx_status_t xhci_continue_transfer_locked(xhci_t* xhci, xhci_endpoint_t* ep, iotxn_t* txn) {
    ...
    while (state->remaining > 0 && free_trbs > 0) {
        ...
        size_t phys_length;
        xhci_get_phys(state, &phys_addr, &phys_length);
        ...
        uint32_t td_size = state->sg_end - state->sg; // number of packets remaining after this one
    ...
    XHCI_WRITE32(&xhci->doorbells[proto_data->device_id], state->ep_index + 1);
