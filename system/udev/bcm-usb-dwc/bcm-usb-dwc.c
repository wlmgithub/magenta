...
mx_status_t dwc_hub_device_added(mx_device_t* hci_device, uint32_t hub_address, int port,
    ...
    iotxn_t* get_desc;
    mx_status_t status = iotxn_alloc(&get_desc, IOTXN_ALLOC_CONTIGUOUS, 64);
    ...
    iotxn_t* set_addr;
    status = iotxn_alloc(&set_addr, IOTXN_ALLOC_CONTIGUOUS, 64);
    assert(status == NO_ERROR);
    ...
    while (true) {
        ...
        if (usb_ep_type(&self->desc) == USB_ENDPOINT_CONTROL) {
            ...
            case CTRL_PHASE_SETUP:
                ...
                mx_status_t status =
                    iotxn_alloc(&req->setuptxn, IOTXN_ALLOC_CONTIGUOUS, sizeof(usb_setup_t));
                assert(status == NO_ERROR);
