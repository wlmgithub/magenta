...
static mx_status_t ahci_bind(mx_driver_t* drv, mx_device_t* dev, void** cookie) {
    ...
    status = pci->get_config_vmo(dev, &pci_res);
    ...
    if (status != NO_ERROR) {
        xprintf("ahci: error %d setting bar cache policy\n", status);
        goto fail;
    }

    status = mx_vmar_map(mx_vmar_root_self(), 0, pci_res.mmio_handle, 0, PAGE_SIZE,
            MX_VM_FLAG_PERM_READ | MX_VM_FLAG_PERM_WRITE | MX_VM_FLAG_MAP_RANGE, (uintptr_t*)&device->regs);
    if (status != NO_ERROR) {
